...
# fviz_cluster(km3, data = scaledData) + theme_minimal()
...
# just for fun plot, visually compare the agglomerative and divisive solutions
dendextend::tanglegram(hcUp, hcDown, sort = TRUE)

# assign cluster numbers to original data
USArrests$cluster <-
  as.character(hcUp4)   # NOTE: they are in the same order
...
revViridis <- function(n) {
  viridis(n, direction = -1)
}
...
plot(somMap2,
     what = "add",
     type = "pie",
     variable = USArrests$cluster)
# identify cases at each SOM node
plot(
  somMap2,
  what = "add",
  type = "names",
  variable = row.names(USArrests)
)
