...
Xplot(LR$DEV,
      at = seq(from = 300, to = 1100, by = 100),    # breaks, can also use 'pretty' function
      cex = 1.5, pch = 15,                          # size and shape of markers
      jitter = TRUE, amount = 0.2)                  # vertical jitter to avoid overlap

# default clustering for break point
findTh(LR$DEV,
       n = 1,           # number of thresholds, 1 is default but can specify for mvQCA
       hclustm = "complete",
       distm = "manhattan")
...
Xplot(LR$URB, at = pretty(LR$URB),
      xlab = "URB: Urbanisation, threshold at 50%",
      cex = 1.5, pch = 15, jitter = TRUE)

Xplot(LR$LIT, at = pretty(LR$LIT),
      xlab = "LIT: Literacy, threshold at 75%",
      cex = 1.5, pch = 15, jitter = TRUE)

Xplot(LR$IND, at = pretty(LR$IND),
      xlab = "IND: Industrialisation, threshold at 30%",
      cex = 1.5, pch = 15, jitter = TRUE)

Xplot(LR$STB, at = pretty(LR$STB),
      xlab = "STB: Stable government, threshold at 9.5 Cabinets, reverse",
      cex = 1.5, pch = 15, jitter = TRUE)

Xplot(LR$SURV, at = pretty(LR$SURV),
...
LR$STBcs <- calibrate(-LR$STB, type = "crisp", thresholds = -9.5)        # reverse
...
ttLC <- truthTable(LipsetCrisp, outcome = "SURVcs", complete = TRUE, show.cases = TRUE)
ttLC
...
ParsimoniousLipsetCrisp <- minimize(ttLC, include = "?", details = TRUE)
...
ttLCFail <- truthTable(LipsetCrisp, outcome = "~SURVcs", complete = TRUE, show.cases = TRUE)
ttLCFail
...
ParsimoniousLipsetCrispFail <- minimize(ttLCFail, include = "?", details = TRUE)
...
fuzzyDEV <- data.frame(raw = LR$DEV,
                       fuzzy1 = calibrate(LR$DEV, type = "fuzzy",
                                          thresholds = c(400, 550, 1000),
                                          logistic = FALSE, below = 1, above = 1),
                       fuzzy2 = calibrate(LR$DEV, type = "fuzzy",
                                          thresholds = "e=400, c=550, i=1000",
                                          logistic = FALSE, below = 2, above = 2),
                       fuzzy3 = calibrate(LR$DEV, type = "fuzzy",
                                          thresholds = c(400, 550, 1000),
                                          logistic = FALSE, below = 3, above = 3),
                       fuzzyLog = calibrate(LR$DEV, type = "fuzzy",
                                          thresholds = c(400, 550, 1000),
                                          logistic = TRUE))
...
LR$DEVfs <- calibrate(LR$DEV, type = "fuzzy", thresholds = c(400, 550, 900))
LR$URBfs <- calibrate(LR$URB, type = "fuzzy", thresholds = c(25, 50, 65))
LR$LITfs <- calibrate(LR$LIT, type = "fuzzy", thresholds = c(50, 75, 90))
LR$INDfs <- calibrate(LR$IND, type = "fuzzy", thresholds = c(20, 30, 40))
LR$STBfs <- calibrate(LR$STB, type = "fuzzy", thresholds = c(15, 9.5, 5))       # reverse
LR$SURVfs <- calibrate(LR$SURV, type = "fuzzy", thresholds = c(-9, 0, 10))
...
ttLF <- truthTable(LipsetFuzzy, outcome = "SURVfs",
                   incl.cut = 0.8, complete = TRUE, show.cases = TRUE, sort.by = "incl")
ttLF
...
solution1 <- fuzzyand(LipsetFuzzy$DEVfs, LipsetFuzzy$URBfs, LipsetFuzzy$LITfs,
                      LipsetFuzzy$STBfs, LipsetFuzzy$INDfs)
sol1Outcome <- fuzzyand(LipsetFuzzy$DEVfs, LipsetFuzzy$URBfs, LipsetFuzzy$LITfs,
                        LipsetFuzzy$STBfs, LipsetFuzzy$INDfs, LipsetFuzzy$SURVfs)
...
XYplot(x = DEVfs, y = SURVfs, data = LipsetFuzzy,
       relation = "sufficiency",
       clabels = rownames(LipsetFuzzy))

# component of solution 
XYplot(x = "DEVfs*LITfs*STBfs", y = SURVfs, data = LipsetFuzzy,
       relation = "sufficiency",
       clabels = rownames(LipsetFuzzy))

# full solution
XYplot(x = "DEVfs*LITfs*STBfs*~URBfs*~INDfs", y = SURVfs, data = LipsetFuzzy, relation = "sufficiency",
       clabels = rownames(LipsetFuzzy))
XYplot(x = "DEVfs*LITfs*STBfs*URBfs*INDfs", y = SURVfs, data = LipsetFuzzy, relation = "sufficiency",
       clabels = rownames(LipsetFuzzy))
