...
vertices1 <- read.csv("./datasets/W07 Korea1vertices.txt")
edges1 <- read.csv("./datasets/W07 Korea1edges.txt")
# village 2, vertices and edges
vertices2 <- read.csv("./datasets/W07 Korea2vertices.txt")
edges2 <- read.csv("./datasets/W07 Korea2edges.txt")

# Construct and examine networks ====

# construct networks from nodes and edges
net1 <-
  graph_from_data_frame(edges1, directed = FALSE, vertices = vertices1)
net2 <-
  graph_from_data_frame(edges2, directed = FALSE, vertices = vertices2)
...
hist(
  net1measures$degr,
  breaks = 0:max(net1measures$degr),
  main = "Village 1",
  xlab = "Degree",
  ylab = "Villagers"
)
...
hist(
  net2measures$degr,
  breaks = 0:max(net2measures$degr),
  main = "Village 2",
  xlab = "Degree",
  ylab = "Villagers"
)

paste("Degree centralisation, network 1:",
      centr_degree(net1)$centralization)
paste("Betweenness centralisation, network 1:",
      centr_betw(net1)$centralization)
paste("Degree centralisation, network 2:",
      centr_degree(net2)$centralization)
paste("Betweenness centralisation, network 2:",
      centr_betw(net2)$centralization)
...
plot(
  ...
  vertex.label.cex	= 0.8,
  vertex.size = ifelse(degree(net1) >= 10, 25, 15)
) # large for the higher degree
title(main = "Village 1")
...
plot(
  ...
  vetex.label.size = 0.2,
  vertex.label.cex	= 0.8,
  vertex.size = ifelse(degree(net2) >= 10, 25, 12)
) # large for the higher degree
title(main = "Village 2")
legend(
  x = -2,
...
paste("Homophily of adoption, Village 1:",
      round(assortativity_nominal(net1, types = (V(
        net1
      )$Adopted + 1)), digits = 2))
paste("Homophily of adoption, Village 2:",
      round(assortativity_nominal(net2, types = (V(
        net2
      )$Adopted + 1)), digits = 2))
...
hist(
  net1measures$kCore,
  breaks = 0:max(net1measures$kCore),
  main = "Village 1",
  xlab = "k-core",
  ylab = "Villagers"
)
hist(
  net2measures$kCore,
  breaks = 0:max(net2measures$kCore),
  main = "Village 2",
  xlab = "k-core",
  ylab = "Villagers"
)
...
# community detection for village 1, uses edge betwenness
